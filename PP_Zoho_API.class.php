...
class PP_Zoho_API {
	...
	function __construct($authToken=null) {
		...
		$configuration = array(
			...
			'accounts_url' => self::getRegionalUrl('https://accounts.zoho.com'), // woocommerce-and-zoho-crm-connector-pro-debug\vendor\zohocrm\php-sdk\README.md
		);
	...
	public static function getRegionalUrl($url) {
		$defaultDomains = array('zoho.com', 'zohoapis.com');
		
		$altDomains = array(
			'zoho.com.au' => array('zoho.com.au', 'zohoapis.com.au'),
			'zoho.eu' => array('zoho.eu', 'zohoapis.eu'),
			'zoho.in' => array('zoho.in', 'zohoapis.in'),
			'zoho.com.cn' => array('zoho.com.cn', 'zohoapis.com.cn')
		);
		
		$host = get_option('pp_wczc_zoho_host', 'https://crm.zoho.com');
		
		foreach ($altDomains as $primaryDomain => $domains) {
			if ( strpos($host, $primaryDomain) ) {
				return str_replace($defaultDomains, $domains, $url);
			}
		}
